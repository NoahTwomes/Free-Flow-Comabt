...
void UFFCS_EnvQG_GetBackCone::GenerateItems(FEnvQueryInstance& QueryInstance) const
{
	...
	for (int32 CenterIndex = 0; CenterIndex < CenterActors.Num(); CenterIndex++)
	{
		...
		{
			//Inverse the forward vector to get the backward vector
...
{
	return FText::Format(LOCTEXT("ConeDescriptionGenerateAroundContext", " generate behind of {0}"), UEnvQueryTypes::DescribeContext(CenterActor));
}
